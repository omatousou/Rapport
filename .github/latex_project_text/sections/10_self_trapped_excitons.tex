...
\begin{figure}[htbp]
    \centering
    \repfig[0.85\linewidth]{rep_trapping_sequence.png}
    \caption{Free and trapped populations on the axis at the most intense plane of the $5\ \mu$J run, from the pulse out to $2\ \text{ps}$. The free electron density decays with the trapping time $\tau_r$ and feeds the self-trapped exciton population, which rises on that same time constant and then decays on the much longer $\tau_{STE}$. Beyond the vertical dashed line the solver's temporal window has ended and the curves are continued by integrating the population equations analytically; this is legitimate because the field there is below $10^{-10}$ of its peak, so the equations reduce to the linear decay and transfer terms alone. The picosecond-scale separation between the two curves is what a pump--probe delay scan measures directly, since the two populations enter $\Delta n$ with opposite signs.}
    \label{fig:rep_trapping}
\end{figure}
