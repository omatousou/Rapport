...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{comparison_fig13_free_vs_trapped.png}
    \caption{Free electron density $N$ (black, labeled $\rho_e$ libre) and trapped self-trapped exciton density $N_{STE}$ (blue, labeled $\rho_s$ piégé) on-axis at the nonlinear focus, against the local pulse intensity (dashed red). $N$ rises together with the pulse and peaks close to the intensity maximum, then decays as electrons leave the conduction band through the trapping term $N/\tau_r$ of the rate equations in Section~11.2. $N_{STE}$ lags behind, rising as $N$ is depleted and saturating once trapping is essentially complete, the microscopic bond-breaking process of Fig.~\ref{fig:dangling_bonds} converting the transient free-carrier population into the permanent structural modification discussed above.}
    \label{fig:free_vs_trapped}
\end{figure}
