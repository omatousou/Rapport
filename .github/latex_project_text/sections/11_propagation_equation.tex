...
&\underbrace{\frac{i}{2k_0}\,\hat{U}^{-1}\nabla_\perp^2 u}_{\text{diffraction + space-time focusing}}
\;+\;
\underbrace{i\,\hat{D}\,u}_{\substack{\text{dispersion: exact Sellmeier, all orders}\\
...
First, the diffraction term carries $\hat{U}^{-1}$, not a bare $1/2k_0$. The effective diffraction wavenumber is $k_0\hat{U}(\Omega)$ rather than a fixed $k_0$, which is what couples the transverse and temporal dynamics and is known as space-time focusing. Keeping $\hat{T}^2$ on the Kerr term while dropping $\hat{U}^{-1}$ here would overestimate self-steepening by roughly a factor of two, since the physically meaningful combination is $\hat{T}^2/\hat{U}\simeq\hat{T}$: for fused silica at $800\ \text{nm}$, $n_g/n_0=1.0095$, so the two operators agree to within one percent and the literature form with a single $\hat{T}$ is recovered exactly when both are kept.
