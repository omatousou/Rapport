...
\authornote{The solver currently sits between the two forms above. Its linear half step implements Eq.~\eqref{eq:master_divided} correctly, applying \texttt{inv\_U\_op} to the Laplacian and leaving \texttt{delta\_k} bare, but its nonlinear step applies $\hat{T}^2$ and $\hat{T}$ without the accompanying $\hat{U}^{-1}$, i.e. the numerators of form~\eqref{eq:master_propagation} with the denominator of neither. Every nonlinear term therefore carries a spurious factor $\hat{U}=1+\Omega/\omega_0$. This is $0.3\%$ for the unbroadened input pulse, but reaches a factor of two on the far blue wing of a strongly self-phase-modulated spectrum, which is exactly where self-steepening is measured. The figures produced before this is fixed remain valid for the pulse core and for every effect that does not depend on the far spectral wings.}
