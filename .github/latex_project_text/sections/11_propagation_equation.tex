...
The electric field is written as a carrier wave modulated by a slowly varying envelope $u(r,t,z)$, and the wave equation is reduced under the slowly varying envelope approximation. For fused silica, the propagation equation solved by the code reads:
\begin{align}
...
&+\;\underbrace{i\,\frac{\omega_0}{2n_0c\,\rho_c}\frac{\omega_0^2}{\omega_{tr}^2-\omega_0^2}\,N_{STE}\,u}_{\text{bound STE polarizability, see Section~11.4}}
\end{align}
...
\begin{align}
\frac{dN}{dt} =\;
&\underbrace{W_{PI}(|u|)\left(1-\frac{N}{N_{at}}\right)}_{\text{direct photoionization, depletes available sites}} \nonumber\\[4pt]
...
\begin{equation}
\mathcal{P}_{ion} = U_i\,\left(1-\frac{N}{N_{at}}\right)W_{PI}(|u|)
\end{equation}
...
\begin{equation}
\alpha_{ion} = \frac{U_i\,W_{PI}(|u|)}{n_0c\varepsilon_0|u|^2/2}\left(1-\frac{N}{N_{at}}\right)
\end{equation}
which is exactly the \texttt{photo} variable computed inside \texttt{NonlinearOperator\dotb split}.
