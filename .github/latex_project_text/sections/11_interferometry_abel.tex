...
Everything measured in this work is measured through a probe beam. No free electron density, no exciton population, no intensity is ever recorded directly inside the bulk sample: what a detector sees is the phase and the amplitude that a probe pulse carries away after crossing the excited region. What can be measured is the phase shift accumulated by a probe beam that crosses the excited region, using a Nomarski interferometer. This section derives how that phase shift relates to the refractive index change predicted by the simulation, and describes how \texttt{unified\_filament\_slider\_v3.py} implements both directions of this link, from a measured interferogram back to a refractive index profile, and from a simulated refractive index profile forward to a predicted interferogram. The physical setup and the mathematics of the Abel transform below are identical whether the excited medium is air or fused silica, only the propagation equation that produces the refractive index change differs, and that equation is the subject of Section~\ref{sec:propagation}.
...
\begin{figure}[htbp]
    \centering
    \repfig[\linewidth]{rep_abel_illustration.png}
    \caption{The forward Abel transform applied to one plane of the $5\ \mu$J simulation. Left: the true local index change $\Delta n(r)$ that the model computes. Right: its integral along the line of sight, expressed as an optical path length in nanometres, which is the quantity the interferometer actually records (the right axis converts it to a phase at $515\ \text{nm}$, where $1\ \text{rad}$ corresponds to $82\ \text{nm}$). The projection is both wider and smoother than the underlying profile: structure concentrated near the axis is diluted over the whole chord. Recovering the left panel from the right is the inversion derived in Section~\ref{subsec:abel_inversion}, and this figure shows why that inversion is ill conditioned near $r=0$, where the projection carries the least information about the profile.}
    \label{fig:rep_abel}
\end{figure}

\subsection{Why a Propagation Model Is Needed}

\label{sec:why_model}

The pipeline above turns a raw interferogram into a two dimensional map of the complex refractive index change: the real part from the phase, the imaginary part from the amplitude, both resolved in the plane transverse to the probe and, through the delay stage, in time. That is the complete experimental output, and it is already enough to see that something structured happens inside the glass, since the maps show a channel far longer than a focal spot, changing sign as the delay is scanned.

What those maps cannot do is say \emph{why}. Three distinct physical channels contribute to the same measured $\Delta n$, with opposite signs and different time constants: the Kerr response, which follows the pump envelope instantly; the free electron plasma, which lowers the index and survives for as long as trapping takes; and the self-trapped excitons, which raise it again and persist far longer. A single phase map is one number per pixel, so it cannot separate three overlapping contributions, and the Abel inversion of Section~\ref{subsec:abel_inversion} recovers a radial profile of the sum, not of its parts.

Nor can the measurement explain what set the channel's length and diameter in the first place. Those follow from the competition between self-focusing and plasma defocusing described in Section~\ref{sec:filamentation}, which is a statement about how the pump propagates, and the probe never sees the pump propagating; it only sees what the pump left behind at one delay.

This is what the next three sections supply. Section~\ref{sec:propagation} writes the propagation equation for the pump together with the population equations that feed it, so that every channel contributing to $\Delta n$ is produced by an explicit mechanism rather than fitted. Section~\ref{sec:numerical} describes how that system is integrated, and Section~\ref{sec:cuda} how it is made fast enough to be run repeatedly. The output is a predicted $\Delta n(r,z,t)$ that can be pushed forward through the same Abel transform and the same instrument response derived above, and compared with the measurement directly, channel by channel. Section~\ref{sec:comparison} does that comparison.
