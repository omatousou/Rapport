...
\;+\;
\underbrace{i\,\hat{D}\Big(\hat{U}+\frac{\hat{D}}{2k_0}\Big)u}_{\substack{\text{dispersion: exact Sellmeier, all orders}\\
...
&+\;\underbrace{i\,\frac{\omega_0}{2n_0c\,\rho_c}\frac{\omega_0^2}{\omega_{tr}^2-\omega_0^2}\,N_{STE}\,u}_{\text{bound STE polarizability, new in Section~\ref{subsec:ste_polarizability}}}
\end{align}
...
\frac{\partial u}{\partial z} = \frac{i}{2k_0}\hat{U}^{-1}\nabla_\perp^2 u
 + i\hat{D}u + \frac{i\,\hat{D}^2}{2k_0}\hat{U}^{-1}u
 + \hat{U}^{-1}\Big\{\text{Kerr} + \text{Raman} + \text{ionization} + \text{plasma} + \text{STE}\Big\},
\end{equation}
where $\hat{U}$ cancels on the first power of $\hat{D}$ alone.

\paragraph{Why the single-$\hat{T}$ form of the review literature follows.} The usual statement, that $\hat{U}\simeq\hat{T}$ for fused silica, is true but is not by itself an argument: it does not say what happens to $\hat{U}^{-1}\hat{T}^2$, which is the combination that actually appears. Expanding it settles the question. Write $\varepsilon=\Omega/\omega_0$, so that $\hat{T}=1+\varepsilon$ exactly, and note that
\begin{equation}
\hat{U} = 1+\frac{k'_\omega\Omega}{k_0} = 1+\frac{n_g}{n_0}\,\varepsilon \equiv 1+g\,\varepsilon,
\qquad g=\frac{n_g}{n_0},
\end{equation}
since $k'_\omega=n_g/c$ and $k_0=n_0\omega_0/c$. Then
\begin{equation}
\hat{U}^{-1}\hat{T}^{2} = \frac{(1+\varepsilon)^2}{1+g\varepsilon}
= \big(1+2\varepsilon+\varepsilon^2\big)\big(1-g\varepsilon+g^2\varepsilon^2-\dots\big)
= 1+(2-g)\,\varepsilon+(1-g)^2\varepsilon^2+O(\varepsilon^3).
\end{equation}
For fused silica at $1030\ \text{nm}$, $n_0=1.4500$ and $n_g=1.4627$ give $g=1.00873$, hence
\begin{equation}
\hat{U}^{-1}\hat{T}^{2} = 1+0.99127\,\varepsilon+7.6\times10^{-5}\varepsilon^2+\dots \;\simeq\; 1+\varepsilon = \hat{T},
\end{equation}
the first-order coefficient differing from that of $\hat{T}$ by exactly $1-g=-0.87\ \%$ and the quadratic term being four orders of magnitude below it. The same expansion gives $\hat{U}^{-1}\hat{T}=1+(1-g)\varepsilon+\dots\simeq1$ for the ionization term. This is where the single-$\hat{T}$ expressions of the review literature come from, and the error committed in using them is $(g-1)\varepsilon$, not something merely assumed small. Applying $\hat{U}^{-1}$ to the Laplacian only, while leaving $\hat{T}^2$ on the Kerr term, is \emph{not} one of these two forms and leaves every nonlinear term with one power of $\hat{T}$ too many.

\paragraph{The $\hat{D}^2/2k_0$ term is dropped by the solver.} Equation~\eqref{eq:master_divided} carries a third linear term, $i\hat{D}^2\hat{U}^{-1}u/2k_0$, which comes from expanding the exact dispersion relation $k_z=\sqrt{k(\omega)^2-k_\perp^2}$ beyond first order in $\hat{D}$. The code does not implement it: \texttt{LinearOperator.half\_linear} applies the phase $\hat{D}-k_\perp^2\hat{U}^{-1}/2k_0$, that is the first power of $\hat{D}$ only. The relative size of the omitted term is $|\hat{D}|/2k_0$, which for $\lambda_0=1030\ \text{nm}$ is $2.0\times10^{-4}$ at $800\ \text{nm}$, $1.6\times10^{-3}$ at $600\ \text{nm}$ and $1.1\times10^{-2}$ at $400\ \text{nm}$. It is therefore below a percent everywhere the pulse carries appreciable energy, and only becomes visible on a far blue wing that the spectral mask has already attenuated. Keeping it in Eq.~\eqref{eq:master_propagation} while noting that the solver truncates it is more honest than writing the truncated equation and calling it exact.
...
The photoionization loss term removes energy from the field at exactly the rate needed to promote electrons to the conduction band at the Keldysh rate $W_{PI}$, throttled by the fraction $1-N/N_{at}$ of ionizable sites still neutral, consistent with the ionization loss derivation of Section~\ref{subsec:photoion_loss} below. $N_{at}$ is the total density of ionizable sites (\texttt{rho\_max} in the code) and $N$ is the current free electron density; in the code this factor is \texttt{depl\_field = clip(1 - rho/rho\_max, 0, 1)}. The plasma absorption and plasma induced defocusing are grouped into a single term above since the code computes them together as the real and imaginary parts of the same Drude response derived in Section~\ref{sec:plasma}, with $\sigma_\omega$ the inverse Bremsstrahlung cross section already introduced in Section~\ref{sec:avalanche}.
...
\label{subsec:rate_equations}

The code integrates two coupled populations, the free electron density $N$ and the self-trapped exciton density $N_{STE}$, exactly as introduced in Section~\ref{sec:avalanche}:
\begin{align}
\frac{dN}{dt} =\;
...
\begin{figure}[htbp]
    ...
    \caption{On-axis electron populations at the nonlinear focus for the $800\ \text{nm}$, $1.1\ \mu$J experiment, reintegrating the 0D rate equations above from the saved intensity trace $I(t)$ under three models (MPI only, $+$avalanche, $+$avalanche$+$trapping) and superposing the actual $N(t)$ the CUDA kernel computed on-axis (open circles). The full model coincides with the CUDA output by construction, which is what this overlay is meant to check, and the agreement confirms the exponential-integrator time stepping of Section~\ref{sec:numerical} reproduces the same rate equations solved here analytically term by term. Trapping into $N_{STE}$ (green) becomes the dominant channel within a few hundred femtoseconds of the pulse peak, draining $N$ (dashed black) down from its avalanche-augmented maximum, consistent with the trapping time $\tau_r$ used throughout this report.}
...
\begin{figure}[htbp]
    ...
    \caption{On-axis electron density along $z$ for six input energies from $0.25$ to $1.25\ \mu$J, reproducing Fig.~9 of~\cite{PhysRevB.71.125435}. The dashed lines mark $1$ and $2\times10^{20}\ \text{cm}^{-3}$ as reference levels. As with the peak intensity, every above-threshold curve saturates near a common density once the local intensity reaches $I_{clamp}$, the electron-density signature of the same clamping balance discussed in Section~\ref{sec:filamentation}.}
...
\begin{figure}[htbp]
    \centering
    \repfig[0.85\linewidth]{rep_energy_budget.png}
    \caption{Cumulative energy fractions removed from the pulse along $z$ for the $5\ \mu$J, $1030\ \text{nm}$ run, separated into the two loss terms of Eq.~\eqref{eq:master_propagation}: the photoionization term, which costs $U_i$ per pair created, and the Drude term, which heats carriers that are already free. Their ratio answers a question the transmission measurement alone cannot: whether the depth of the measured absorption dip is set by how many carriers are made or by how hard the existing ones are driven. The total is the transmission deficit that Section~\ref{sec:comparison} compares against the imaginary part of the measured index.}
    \label{fig:rep_energy_budget}
\end{figure}

Note the correspondence with Section~\ref{sec:avalanche}. There, we wrote the same pair of equations with $\sigma/E_g$ in place of $\sigma_\omega/U_i$ and $\tau_x$ in place of $\tau_r$. These are the same quantities, only the notation has been aligned here with the variable names used in the code (\texttt{tau\_r} for the trapping time, \texttt{avalanche\_coef} for $\sigma_\omega/U_i$).

\paragraph{Update relative to the July solver.} The July code only let the self-trapped exciton population decay by re-ionization back into the conduction band, the $\big[W_{PI}(|u_x|)+\beta_sIN\big]N_{STE}/N_{at}$ term above. The parametrization referenced in the code (Sakurai et al.) tabulates for fused silica both a trapping time of $150\ \text{fs}$, already present as $\tau_r$, and a separate relaxation time of the trapped state itself of about $1\ \text{ps}$, which was missing. \texttt{deliverable~(3)} adds an optional term $-N_{STE}/\tau_{STE}$ to the $dN_{STE}/dt$ equation, controlled by \texttt{Config.tau\_ste}, defaulting to \texttt{None} which reproduces the July behavior exactly. This mainly matters for the probe phase at picosecond delays, since it changes how long the STE population, and therefore the index change it produces (Section~\ref{subsec:ste_polarizability}), survives after the pump pulse has left.

\subsection{Photoionization Energy Loss}
\label{subsec:photoion_loss}
...
\label{subsec:ste_polarizability}

This subsection documents a term that did not exist at all in the July solver and is new in \texttt{deliverable~(3)}. A self-trapped exciton is not a conduction band carrier. It is a pair of dangling bonds, $\text{Si}^\bullet$ and $\text{O}^\bullet$, left behind after the Si-O-Si bond breaks, described in Section~\ref{sec:ste}, and its unpaired orbitals sit inside the gap rather than in the conduction band. It therefore does not drift in the field and does not contribute to the free carrier Drude response of Section~\ref{sec:plasma}. What it does instead is polarize as a bound oscillator with a resonance at the energy of its first excited level, following Mao et al.~\cite{mao2004}:
\begin{equation}
