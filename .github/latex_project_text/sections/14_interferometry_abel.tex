...
The script \texttt{web/abel\_phase\_explorer.py} implements both directions of the link between the simulation and a real interferometric measurement, so that the two can be compared directly.
...
\subsubsection{Which Index Model the Probe Uses}

The probe analysis does not reuse the single band STE formula of Section~11.4. It evaluates the full dielectric function of Martin et al.~\cite{martin1997} at the probe wavelength, implemented in \texttt{sim/permittivity.py}, which differs from the pump side model on four points. It uses two STE bands with their own oscillator strengths and widths instead of one. It uses an effective mass of $0.5\,m_e$ in the Drude term rather than the bare mass. It subtracts the valence oscillators that ionization has removed from the ground state, a term proportional to the number of electrons promoted, which the pump side model ignores. And it takes $n=\sqrt{\varepsilon}$ instead of the first order expansion, which matters because the expansion assumes $N\ll N_{at}$ and that condition is violated near the clamping density.

The practical consequence is that phase and absorption now come out of the same complex permittivity, so they cannot contradict each other. Writing $\varepsilon_2$ for the permittivity of the excited medium and $\varepsilon_1$ for the unexcited one, the two observables are
\begin{equation}
\Delta n = \Re\sqrt{\varepsilon_2}-\Re\sqrt{\varepsilon_1},
\qquad
\alpha = \frac{2\omega}{c}\,\Im\sqrt{\varepsilon_2},
\end{equation}
and the code returns both from a single call. The phase map is the Abel transform of $\Delta n$, and the transmittance map is $\exp(-\int\alpha\,dx)$ along the same line of sight.

Each contribution is Abel transformed and low pass filtered separately, by channel. This works because both operations are linear, so summing the channels commutes with both the line of sight integral and the filter. The point of doing it this way is that the browser can then switch individual channels on and off by summing whichever ones are ticked, with no transform to redo. The three channels are the Drude response of the free electrons, which is negative, the bound STE response, which is positive, and the cross Kerr response to the local intensity, which is also positive. The valence depletion term is folded into the Drude channel, since both scale with the number of electrons removed from the valence band.
