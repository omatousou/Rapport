...
a.text(z[-1]*0.5, t_sub[-1]*0.62, "solver temporal window",
...
a.set_ylabel(r"local pump time  $t$ [fs]", fontsize=11)
...
a.text(z[-1]*0.42, -lim*1.32,
       f"the pump needs {z[-1]/vg*1e3:.0f} fs to cross the box\n"
       "dotted : outside the window, analytic continuation",
...
b.set_title(r"the cut yields a map : $\rho_e(z,r)$ at $\tau=1000$ fs"
